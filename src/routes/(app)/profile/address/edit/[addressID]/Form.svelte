<script lang="ts">
	...
	const getSelectedCityOptionUsingPostalCode = (postal_code: string) =>
		CITY_OPTIONS.find((city) => city.postal_code === postal_code) ?? CITY_OPTIONS[0];

	let street_line1 = address.street_line1;
	let street_line2 = address.street_line2;
	let seletedCityOption: CityOption = getSelectedCityOptionUsingPostalCode(address.postal_code);
	let loading = false;
	...
	const submitHandler = async () => {
		try {
			...
			const docRef = doc(db, 'addresses', address.id ?? '');
			await updateDoc(docRef, new_address);
